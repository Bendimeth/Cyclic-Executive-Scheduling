...
export class TaskGroup {
  ...
  public SerializationCondition(): number {
    let n = 0;
  ...
  private TaskToPartPeriod(): void {
    ...
    for (var t of this.taskList) {
      ...
      for (var part of this.partPeriods) {
        ...
        if (timeRange % t.period === 0) {
          const range = timeRange / this.minPeriod;
          const iterator = t.period / this.minPeriod;
          const startIterator = range - iterator;
          const endIterator = range;
          //console.log('start iterator: ' + startIterator + '      task: ' + t.name);
          for (var _i = startIterator; _i < endIterator; _i++) {
            if (this.partPeriods[_i].CheckFreeSpace(t.processingTime)) {
              this.partPeriods[_i].tasks.push(t);
              break;
            }
          }
        }
